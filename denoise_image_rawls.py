...
def main():
    ...
    parser.add_argument('--features', type=str, help="images input path (ex: \"image1.rawls image2.rawls image3.rawls\")")
    ...
    for img_path in p_features:
        ...
        img_array = np.array(Rawls.load(img_path))
    ...
    for i in range(n_h_tiles):
        for j in range(n_w_tiles):
            ...
            # TODO : renormalize tile using whole image
            dim_sum = 0

            # normalize for each kind of image
            for dim in n_dims:
                input_tile[dim_sum:dim_sum + dim] = min_max_norm(input_tile[dim_sum:dim_sum + dim], input_data[dim_sum:dim_sum + dim])
